...
OctTNode::OctTNode(int nid,NodeType type,double* low,double* high,int father){
	n_id = nid;
	n_type = type;
	n_ptCount = 0;
	for(int i=0;i<3;i++){
...
void OctTNode::storeToByteArray(byte **pdata, uint32_t &len) {
...
{
	for(int i=0; i<3; i++)
		if(pt->p_xyz[i] < n_domLow[i]-eps || pt->p_xyz[i] > n_domHigh[i]+eps)
		{
			return false;
		}
		return true;
...
void OctTNode::octsplit(double* slow, double* shigh, char idx, double* low, double* high)
{
...
void OctTNode::calOverlap(double domLow[],double domHigh[]){

};
//change the domLow/high to overlap domLow/high with this node
// void OctTNode::nodeInsert(OctPoint *pt){cout<<"base insert!"<<endl;}; 
// void OctTNode::nodeAjust(){cout<<"base adjust!"<<endl;}; 
// void  OctTNode::nodeSplit(){cout<<"base split!"<<endl;}; 
