...
OctLeafNode::OctLeafNode(int nid,NodeType type,double* low,double* high,int father){
	n_id = nid;
	n_type = type;
	n_ptCount = 0;
	for(int i=0;i<3;i++){
...
void OctLeafNode::geneMidpoint(IDTYPE pid1,IDTYPE pid2,int condition){
	...
	double time = (g_PtList.find(pid1)->second->p_time + g_PtList.find(pid2)->second->p_time ) / 2.0;
	...
	if(condition == 1 || condition ==0){
		...
		calMid(g_PtList.find(pid1)->second->p_xyz,g_PtList.find(pid2)->second->p_xyz,add);
...
IDTYPE OctLeafNode::whichIsCloser(IDTYPE pid1,IDTYPE pid2,IDTYPE pid){
	if(abs(g_PtList.find(pid1)->second->p_xyz[0]-g_PtList.find(pid)->second->p_xyz[0]) > abs(g_PtList.find(pid2)->second->p_xyz[0]-g_PtList.find(pid)->second->p_xyz[0]))
		return pid2;
	else return pid1;
}
...
bool OctLeafNode::split_or_not(){
	...
	for(int i=0; i<8; i++){
		if( threshold <= ((double)point_count_in_children[i]) / ((double)n_ptCount) ){
			return false;
		}
	}
	return true;
...
void OctLeafNode::nodeSplit(){
	...
	*/
	...
	while(!data.empty()){
		...
		while( 
			( (pre!=0) && data.find(pre)!=data.end() && (bool_pre = containPoint( g_PtList.find(pre)->second,top_in_which_child ) )) ||
			( (next!=0) && data.find(next)!=data.end() && (bool_next = containPoint( g_PtList.find(next)->second,top_in_which_child) ))
			)//point in the same trajectory 
		{
			if(pre!=0 && data.find(pre)!=data.end() && (bool_pre = containPoint( g_PtList.find(pre)->second,top_in_which_child ) ) ){
				//add
				splitPointIntoChild(g_PtList.find(pre)->second,top_in_which_child);
				pre_next = pre;
			...
			if(next!=0 && data.find(next)!=data.end()&& (bool_next = containPoint( g_PtList.find(next)->second,top_in_which_child) ) ){
				//add
				splitPointIntoChild(g_PtList.find(next)->second,top_in_which_child);
				next_pre = next;
		...
		int condition;
		if(pre!=0 && data.find(pre)!=data.end()&&(condition=judgeBetween(pre,pre_next))>1){
			geneMidpoint(pre,pre_next,condition);
			data.erase(pre_next);
		}
		if(next!=0&& data.find(next)!=data.end()&&(condition=judgeBetween(next_pre,next))>1){
...
void OctLeafNode::convertToIndex(){
	//use the same n_id
	OctIdxNode *newnode = new OctIdxNode(n_id,IDX,n_domLow,n_domHigh,n_father);
	for(int i=0;i<8;i++){
