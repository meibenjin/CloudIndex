...
int do_create_torus(struct message msg) {
    ...
    if(the_torus == NULL) {
       ...
       //create long socket fd from current node to result collecter
        ...
        // set reply string and local_ip
        reply_len = sprintf(reply, "reply from %s: initialize info received.\n", the_torus->info.ip);

    } else {
        reply_len = sprintf(reply, "reply from %s: initialize info has been received before.\n", the_torus->info.ip); 
    }

    struct message rep_msg;
    rep_msg.msg_size = calc_msg_header_size() + reply_len; 
    fill_message(rep_msg.msg_size, REP_CREATE_TORUS, the_torus->info.ip, result_ip, \
                     "", reply, reply_len, &rep_msg);

    send_data(result_sockfd, (void *) &rep_msg, rep_msg.msg_size);
...
int do_new_skip_list(struct message msg) {
    ...
    if(the_skip_list == NULL) {
        ...
        // set reply string and local_ip
        reply_len = sprintf(reply, "reply from %s: route info received.\n", the_torus->info.ip);

    } else {
        reply_len = sprintf(reply, "reply from %s: route info has been received before.\n", the_torus->info.ip); 
    }

    struct message rep_msg;
    rep_msg.msg_size = calc_msg_header_size() + reply_len; 
    fill_message(rep_msg.msg_size, REP_NEW_SKIP_LIST, the_torus->info.ip, result_ip, \
                     "", reply, reply_len, &rep_msg);
...
int forward_search(int op, struct interval intval[], struct message msg, int d) {
	...
	if (lower_neighbor != NULL) {
		...
		fill_message(msg.msg_size, (OP)msg.op, src_ip, dst_ip, msg.stamp, \
...
int send_oct_points(const char *dst_ip, hash_map<int, OctPoint *> &points) {
    ...
    fill_message(msg.msg_size, LOAD_OCT_TREE_POINTS, local_ip, dst_ip, "", \
                 "", 1, &msg);
...
int send_oct_nodes(const char *dst_ip, hash_map<int, OctTNode *> &nodes) {
    ...
    struct message msg;
    msg.msg_size = calc_msg_header_size() + 1;
    fill_message(msg.msg_size, LOAD_OCT_TREE_NODES, local_ip, dst_ip, "", \
                 "", 1, &msg);
    send_data(socketfd, (void *)&msg, msg.msg_size);
...
int find_idle_torus_node(char idle_ip[][IP_ADDR_LENGTH], int requested_num, int* actual_got_num) {
    ...
    *actual_got_num = 0;
    ...
    if (is_active_leader() == TRUE){
        ...
        cpy_len += sizeof(int);
        memcpy(msg.data + cpy_len, &requested_num, sizeof(int));
        cpy_len += sizeof(int);
...
int calc_nn_query_refinement(struct query_struct query, struct traj_segments traj_segs[], uint32_t traj_num, double *avg_qp) {
    ...
    cpy_len += sprintf(buffer + cpy_len, "nn query %d:", query.data_id);
    for (i = 0; i < MAX_DIM_NUM; i++) {
        cpy_len += sprintf(buffer + cpy_len, "(%lf, %lf) ", query.intval[i].low, query.intval[i].high);
    }
    ...
    fill_message(qry_msg.msg_size, REP_NN_QUERY, the_torus->info.ip, result_ip, \
...
int send_refinement_request(hash_map<IDTYPE, Traj*> &trajs, struct query_struct query, char *dst_ip){
    ...
    msg.msg_size = calc_msg_header_size() + len;
    send_data(socketfd, (void *)&msg, msg.msg_size);

    // begin to send points to idle_ip
    send_data(socketfd, (void *)data, data_size);
...
int send_throw_out_query_msg(struct query_struct query, struct refinement_stat overall_stat, char *reason) {
    ...
    send_data(result_sockfd, (void *) &notify_msg, notify_msg.msg_size);
    return TRUE;
}
...
int local_oct_tree_query(struct query_struct query, double low[], double high[]) {
    ...
    if (RUNNING_MODE == 0){
        ...
        msg.msg_size = calc_msg_header_size() + sizeof(struct filter_log_struct);
        send_data(result_sockfd, (void *) &msg, msg.msg_size);
    } else {
    ...
    if(query.op == DATA_INSERT) {
		...
		if(the_torus_oct_tree->containPoint(point)){
            ...
            if(count % freq == 0) {
                ...
                size_t reply_len = 0;
                reply_len = snprintf(reply, BUF_SIZE, "%s:%lu points inserted.\n", \
                        the_torus->info.ip, count);

                msg.msg_size = calc_msg_header_size() + reply_len;
                fill_message(msg.msg_size, REP_INSERT_DATA, the_torus->info.ip, \
                        result_ip,"", reply, reply_len, &msg);
...
int do_nn_query_refinement(connection_t conn, struct message msg) {
    ...
    if (RUNNING_MODE == 0){
        ...
        res_msg.msg_size = calc_msg_header_size() + sizeof(struct refinement_log_struct);
        send_data(result_sockfd, (void *) &res_msg, res_msg.msg_size);
    } else {
...
int receive_refinement_data(connection_t conn, uint32_t data_size, struct traj_segments traj_segs[], uint32_t traj_num) {
    ...
    while(loop){
        length = recv_data(conn->socketfd, buf, data_size);
        if(length <= 0) {
...
int do_range_query_refinement(connection_t conn, struct message msg) {
    ...
    cpy_len += sprintf(buffer + cpy_len, "range query %d:", query.data_id);
    for (i = 0; i < MAX_DIM_NUM; i++) {
        cpy_len += sprintf(buffer + cpy_len, "(%lf, %lf) ", query.intval[i].low, query.intval[i].high);
    }
    cpy_len += sprintf(buffer + cpy_len, "\n{\n");
    ...
    struct message qry_msg;
    qry_msg.msg_size = calc_msg_header_size() + cpy_len; 
    fill_message(qry_msg.msg_size, REP_RANGE_QUERY, the_torus->info.ip, result_ip, \
                     "", buffer, cpy_len, &qry_msg);
...
int replicate_data(struct message msg) {
    ...
    if(replica_buf_offset >= FLUSH_SIZE) {
        ...
        for (i = 0; i < num; i++){
            struct message new_msg;
            new_msg.op = REPLICA_DATA;
            strncpy(new_msg.src_ip, src_ip, IP_ADDR_LENGTH);
            strncpy(new_msg.dst_ip, dst_ips[i], IP_ADDR_LENGTH);
            strncpy(new_msg.stamp, msg.stamp, STAMP_SIZE);
    ...
    if(replica_buf_offset < FLUSH_SIZE) {
        ...
        cpy_len += sizeof(int);
        struct query_struct item;
        memcpy(&item, msg.data + cpy_len, sizeof(struct query_struct));
        replica_buf[replica_buf_offset++] = item;
    }
...
int write_global_properties() {
    ...
    len += sprintf(buf + len, "NUM_REPLICAS=%u\n", NUM_REPLICAS);
...
int write_to_disk(char *buf, size_t data_len, char *replica_ip) {
	FILE *fp;
    char file_name[MAX_FILE_NAME];
    sprintf(file_name, "%s/replica_%s", REPLICAS_DIR, replica_ip); 
	fp = fopen(file_name, "ab+");
    if(fp == NULL) {
        //TODO write error log
        write_log(ERROR_LOG, "write_to_disk:open file failed.\n");
        return FALSE;
    }
...
int do_replica_data(struct message msg) {
    ...
    if(buff_size[index] < 1024) {
        int cur_len = sizeof(query_struct) * FLUSH_SIZE * buff_size[index];
        memcpy(write_file_buffer[index] + cur_len, msg.data, sizeof(query_struct) * FLUSH_SIZE);
        buff_size[index]++;
    } else {
        write_to_disk(write_file_buffer[index], sizeof(query_struct) * FLUSH_SIZE * buff_size[index], msg.src_ip);
        buff_size[index] = 0;
    }
    return TRUE;
}
...
int to_forward_direction(struct query_struct query, int index){
    ...
    if(interval_overlap(sln_ptr->level[0].forward->leader[index].region[2], \
                        query.intval[2]) != 0)
    {
        return FALSE;
    }
...
int to_backward_direction(struct query_struct query, int index){
    ...
    if (interval_overlap(sln_ptr->level[0].backward->leader[index].region[2],\
                         query.intval[2]) != 0) 
    {
        return FALSE;
    }
...
void hand_on_query_to_neighbour(struct message msg, int direction, const char *dst_ip, int index) {
    ...
    memset(stamp, 0, STAMP_SIZE);
    if(direction == 1) {
        strncpy(stamp, "forward", STAMP_SIZE);
        sockfd = sln_forward_sockfd[0][index]; 
    } else {
        strncpy(stamp, "backward", STAMP_SIZE);
        sockfd = sln_backward_sockfd[0][index]; 
    }

    data_len = msg.msg_size - calc_msg_header_size();
    fill_message(msg.msg_size, (OP)msg.op, src_ip, dst_ip, stamp, \
                 msg.data, data_len, &new_msg);

    send_data(sockfd, (void *) &new_msg, new_msg.msg_size);
}
...
void hand_on_query(struct message msg, struct query_struct query, int index) {
    ...
    if(strcmp(msg.stamp, "") == 0) {
        if (to_forward_direction(query, index) == TRUE) {
        ...
        if (to_backward_direction(query, index) == TRUE) {
...
int do_query_torus_cluster(struct message msg) {
	...
	sln_ptr = the_skip_list->header;
    if(sln_ptr == NULL) {
        write_log(TORUS_NODE_LOG, "the_skip_list is null\n");
        return FALSE;
    }
	...
	if (0 == interval_overlap(sln_ptr->leader[index].region[2], query.intval[2])) { // node searched
        ...
        // node is on the forward of skip list
		int visit_forward = 0;
        ...
        // node is on the backward of skip list
...
int do_check_system_status(struct message msg) {
    ...
    if(the_torus == NULL) {
        ...
        cpy_len += sprintf(buf + cpy_len, "[%d_%d%d%d] ", node.cluster_id, id.x, id.y, id.z);
        cpy_len += sprintf(buf + cpy_len, "%s:\n{\n", node.ip); 
        cpy_len += sprintf(buf + cpy_len, "\tcapacity:%d,\n", the_torus->info.capacity);

        cpy_len += sprintf(buf + cpy_len, "\tX:(%lf, %lf),\n", \
                the_torus->info.region[0].low, the_torus->info.region[0].high);

        cpy_len += sprintf(buf + cpy_len, "\tY:(%lf, %lf),\n", \
                the_torus->info.region[1].low, the_torus->info.region[1].high);

        cpy_len += sprintf(buf + cpy_len, "\tZ:(%lf, %lf),\n", \
                the_torus->info.region[2].low, the_torus->info.region[2].high);


        int data_num = g_ptCount;
        cpy_len += sprintf(buf + cpy_len, "\tdata points:%d\n", data_num);

        cpy_len += sprintf(buf + cpy_len, "}\n");

        // set local_ip
        strncpy(local_ip, node.ip, IP_ADDR_LENGTH);
    }

    status_msg.msg_size = calc_msg_header_size() + cpy_len;
    fill_message(status_msg.msg_size, REP_CHECK_SYSTEM_STATUS, local_ip, \
                            result_ip, "", buf, cpy_len, &status_msg); 
    send_data(result_sockfd, (void *) &status_msg, status_msg.msg_size);
...
int do_check_insert_data(struct message msg) {
    ...
    if(the_torus == NULL) {
        ...
        // set local_ip
        strncpy(local_ip, the_torus->info.ip, IP_ADDR_LENGTH);
    }

    status_msg.msg_size = calc_msg_header_size() + cpy_len;
    fill_message(status_msg.msg_size, WRITE_INSERT_DATA, local_ip, \
                            result_ip, "", buf, cpy_len, &status_msg); 
...
int do_query_end(connection_t conn, struct message msg) {
    int data_id, cpy_len, res_num;
    double rq_time, rt_time;
    cpy_len = 0;
    memcpy(&data_id, msg.data, sizeof(int));
    cpy_len += sizeof(int);
    memcpy(&rq_time, msg.data + cpy_len, sizeof(double));
    cpy_len += sizeof(double);
    memcpy(&rt_time, msg.data + cpy_len, sizeof(double));
    cpy_len += sizeof(double);

    memcpy(&res_num, msg.data + cpy_len, sizeof(int));
    cpy_len += sizeof(int);
...
int do_throughput_insert(struct message msg) {
    ...
    memcpy(&max_hop, msg.data + cpy_len, sizeof(double));
    cpy_len += sizeof(double);
...
int do_trajectory_query(connection_t conn, struct message msg) {
    ...
    send_data(conn->socketfd, (void *)buf, SOCKET_BUF_SIZE);
    return TRUE;
}
...
int do_seek_idle_node(connection_t conn, struct message msg) {
        ...
        /*if(route_count == 0) {
            ...
            if(backward != NULL) {
                ...
                if (TRUE == backward_sockfd) {
                    ...
                    msg.msg_size = calc_msg_header_size() + sizeof(int);
                    send_data(backward_sockfd, (void *)&msg, msg.msg_size);
    ...
    reply_msg.msg_size = calc_msg_header_size() + cpy_len;
    send_data(conn->socketfd, (void *)&reply_msg, reply_msg.msg_size);
    return TRUE;
}
...
int send_node_status(const char *dst_ip, struct node_stat stat) {
    ...
    msg.msg_size = calc_msg_header_size() + sizeof(struct node_stat);
    send_data(socketfd, (void *)&msg, msg.msg_size);
    return TRUE;
}
...
static int register_message_handler_list() {
    ...
    register_message_handler2(QUERY_TORUS_CLUSTER, &do_query_torus_cluster);
    register_message_handler2(LOAD_DATA, &do_load_data);
    register_message_handler2(REPLICA_DATA, &do_replica_data);
    register_message_handler2(NEW_SKIP_LIST, &do_new_skip_list);
    register_message_handler2(UPDATE_SKIP_LIST, &do_update_skip_list);
    register_message_handler2(UPDATE_SKIP_LIST_NODE, &do_update_skip_list_node);
    ...
    register_message_handler2(CHECK_INSERT_DATA, &do_check_insert_data);
    register_message_handler2(WRITE_INSERT_DATA, &do_write_insert_data);
    register_message_handler2(CHECK_RANGE_QUERY, &do_check_range_query);
    register_message_handler2(CHECK_NN_QUERY, &do_check_nn_query);
    register_message_handler2(CHECK_CONN_BUFFER, &do_check_conn_buffer);
    register_message_handler2(CHECK_TRAJ_SEQ, &do_check_traj_seq);
    register_message_handler2(QUERY_START, &do_query_start);


    // this need connection struct 
    register_message_handler1(SEEK_IDLE_NODE, &do_seek_idle_node);
    register_message_handler1(LOAD_OCT_TREE_POINTS, &do_load_oct_tree_points);
    register_message_handler1(LOAD_OCT_TREE_NODES, &do_load_oct_tree_nodes);
    register_message_handler1(LOAD_OCT_TREE_TRAJECTORYS, &do_load_oct_tree_trajectorys);
    register_message_handler1(TRAJ_QUERY, &do_trajectory_query);
    register_message_handler1(RANGE_QUERY_REFINEMENT, &do_range_query_refinement);
    register_message_handler1(NN_QUERY_REFINEMENT, &do_nn_query_refinement);
    register_message_handler1(QUERY_END, &do_query_end);
    return TRUE;
}
...
int main(int argc, char **argv) {
    ...
    /* wait for heartbeat thread finish
